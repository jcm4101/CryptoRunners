...
namespace XOR_Shellcode
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                decBytes[i] = (byte)((uint)encBytes[i] ^ keyBytes[0]);
            }

            // Format our byte key into a format we can use later
            //string keyStr = FormatByteToHex(keyBytes[0]);

            // Format our byte array into a variable format we can use later

            string keyString = FormatByteArrayToHex(keyBytes, "keyBytes");
