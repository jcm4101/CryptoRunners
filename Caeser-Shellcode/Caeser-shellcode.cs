...
namespace Caesar_Shellcode
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Encrypt by shifting to the right (+) but you can go in either direction to start
            byte[] encBytes = new byte[payload.Length];
            for (int i = 0; i < payload.Length; i++)
            {
                encBytes[i] = (byte)(((uint)payload[i] + key) & 0xFF);
            }
            ...
            {
                decBytes[i] = (byte)(((uint)encBytes[i] - key) & 0xFF);
            }
